...
private:
  ...
  TinyGPSPlus gps;
  static constexpr unsigned long GPS_CACHE_VALIDITY_MS = 1000UL;
  ...
  bool isDaylightSavingTime(int year, int month, int day, int hour) {
    ...
    // April to September -> daylight saving time
    if (month > 3 && month < 10) {
      return true;
    }
  ...
  // Update the cache with fresh Hungarian time
...
public:
  ...
  uint8_t getGPSQuality() {
    double hdop = getHdop();
    if (hdop < 1.0) return 0;
    if (hdop < 2.0) return 1;
    if (hdop < 5.0) return 2;
    if (hdop < 10.0) return 3;
    if (hdop < 20.0) return 4;
    return 5;
  }
  ...
  // Getter functions use cache instead of recalculating
  int getYear() {
    if (!isCacheValid()) {
      updateTimeCache();
    }
